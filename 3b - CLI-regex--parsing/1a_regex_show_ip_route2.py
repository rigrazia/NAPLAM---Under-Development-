...
R     192.168.5.0/24 [120/1] via 192.168.3.2, 00:00:21, GigabitEthernet0/2
                     [120/1] via 192.168.2.2, 00:00:23, GigabitEthernet0/1
'''
...
route_regex = re.compile(
    # Match the start of a new route or continuation of a multipath route
    r'(?:(?P<protocol>L|C|S|R|M|B|D|EX|O|IA|N1|N2|E1|E2|i|su|L1|L2|ia|U|o|P|\+|[*])\*?\s+)?'
    r'(?P<network>\S+/\d+)?\s*'  # Network (with CIDR notation), optional as it may not be present on multipath lines
    # Match the route details, which may be on the same line or a subsequent line for multipaths
    r'(?:\[(?P<admin_distance>\d+)/(?P<metric>\d+)\]\s+)?'  # Administrative distance and metric
    r'(?:via\s+(?P<next_hop>\S+),\s+)?'  # Next-hop
    r'(?:is\s+)?(?P<description>[\w\s-]+,)?\s*'  # Description (e.g., "directly connected"), optional
    r'(?:[0-9]{2}:[0-9]{2}:[0-9]{2},\s+)?'  # Update time, optional
    r'(?P<interface>\S+)?'  # Interface, optional on multipath lines
    r'(?:\s*|\Z)',  # Match spaces or end of input to handle multipath routes
    re.MULTILINE  # MULTILINE mode to match patterns across multiple lines
)
